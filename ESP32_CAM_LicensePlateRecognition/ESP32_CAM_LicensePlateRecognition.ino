...
const char* ssid = "DELT_1101";         // Replace "xxx" with your WiFi SSID
const char* password = "Delt_1101";      // Replace "xxx" with your WiFi Password
...
String apiKey = "cd_bel_110626_0ymjT9";             // Replace "xxx" with your API key
...
// Função corrigida para extrair valores do JSON com segurança
String extractJsonStringValue(const String& jsonString, const String& key) {
  int keyIndex = jsonString.indexOf(key);
  if (keyIndex == -1) return "";

  // Encontra os dois pontos após a chave
  int colonIndex = jsonString.indexOf(':', keyIndex);
  if (colonIndex == -1) return "";

  // Encontra a primeira aspas após os dois pontos (início do valor)
  int startQuote = jsonString.indexOf('"', colonIndex);
  if (startQuote == -1) return "";

  // Encontra a aspas final (fim do valor)
  int endQuote = jsonString.indexOf('"', startQuote + 1);
  if (endQuote == -1) return "";

  // Retorna o conteúdo exatamente entre as aspas
